      ...
      <div className="mb-4">
        ...
        <div className="flex items-center gap-3 p-3 border border-green-200 dark:border-green-800 rounded-xl bg-green-50 dark:bg-green-900/20">
          ...
          <button
            type="button"
            onClick={handleClear}
            className="cursor-pointer text-[var(--text-muted)] hover:text-[var(--text-primary)] text-sm font-medium"
          >
            Change
          </button>
        </div>
  ...
  return (
    <div className="mb-4 relative">
      ...
      {companyOpen && companyHits.length > 0 && (
        <ul className="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-[var(--surface-border)] bg-[var(--surface-card)] shadow-lg">
          {companyHits.map((item) => (
            <li
              key={item.number}
              className="cursor-pointer px-3 py-2 text-sm hover:bg-[var(--surface-elevated)] transition-colors"
